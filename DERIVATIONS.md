...
$$
-\frac{\alpha\Delta t}{2\Delta r^2}
+
\frac{\alpha\Delta t}{4r_j\Delta r}
$$
...
$$
-\frac{k}{\Delta r}T_s^{n+1}
+
\frac{k}{\Delta r}T_{N-2}^{n+1}
=
hT_s^{n+1}
-
hT_\infty
...
## 15. Comparison of the Numerical Methods

### Explicit Finite Difference

The Explicit method calculates the new temperature directly from the previous time level.

Advantages:

- Simple formulation
- Straightforward implementation
- No simultaneous equation solution required

Limitations:

- Conditionally stable
- Time step is restricted by the spatial grid and thermal diffusivity
- Fine spatial grids can require very small time steps

### Crank-Nicolson

The Crank-Nicolson method couples the temperatures at the new time level through a system of simultaneous equations.

Advantages:

- Unconditionally stable for the linear diffusion problem
- Second-order accurate in time for the standard Crank-Nicolson formulation
- Allows larger time steps without the Explicit stability restriction

Limitations:

- Requires a linear system to be solved at every time step
- More computationally expensive per time step
- A stable solution is not necessarily an accurate solution if the selected time step is excessively large

## 16. Summary

The numerical model therefore consists of:

1. A uniform radial grid extending from the centreline to the outer surface.
2. An Explicit finite difference solution with special treatment at the cylindrical centreline.
3. A Crank-Nicolson solution expressed as a matrix system.
4. A symmetry boundary condition at the centreline.
5. A convective boundary condition at the resistor surface.
6. Uniform internal volumetric heat generation.
7. Explicit stability assessment using the Fourier number.

The two independent numerical implementations provide a means of comparing the predicted transient temperature distributions and evaluating the behaviour of the numerical methods.